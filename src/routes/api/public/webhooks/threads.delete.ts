/**
 * Threads Data Deletion callback — Meta POSTs a signed_request here when a
 * Threads user requests deletion of their data. We delete their Threads
 * account row plus Threads publishing history.
 *
 * URL: https://postspark.co/api/public/webhooks/threads/delete
 */
...
export const Route = createFileRoute("/api/public/webhooks/threads/delete")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const confirmationCode = Buffer.from(`del-${threadsUserId}`).toString("hex");
        return new Response(
          JSON.stringify({
