...
import { Clock, FileText, Download, Star, Search, CheckSquare, Square, Globe, Copy, UserCheck, Trash2, RefreshCw, Sparkles } from "lucide-react";
    ...
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      <div className="mt-4 relative">
        ...
        <input
          ...
          placeholder="Search by title, input, or prompt..."
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        >
          ...
          {availableTools.map((t) => (
            <option key={t} value={t}>{TOOL_LABEL[t] || t}</option>
          ))}
        </select>
        <select
          value={filterModel}
        ...
        <div className="flex items-center gap-1 rounded-md border border-input bg-background p-0.5">
          {([
            ...
            <button
              ...
              className={`rounded px-2 py-1 text-[11px] font-medium transition ${filterRange === k ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-accent"}`}
            >
              {l}
            </button>
          ))}
        </div>
        {(filterTool !== "all" || filterModel !== "all" || filterRange !== "all" || filterFav) && (
          <button
            onClick={() => { setFilterTool("all"); setFilterModel("all"); setFilterRange("all"); setFilterFav(false); }}
            className="text-[11px] text-primary hover:underline"
          >Clear filters</button>
        )}
        <span className="ml-auto text-[11px] text-muted-foreground">{filteredJobs.length} of {jobs.length}</span>
      </div>
